...
public class lab_update_vaccination_result_page extends JFrame {
	...
	private static JTable vaccinationtable;
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					lab_update_vaccination_result_page frame = new lab_update_vaccination_result_page();
	 ...
	 * Create the frame.
	 */
	public lab_update_vaccination_result_page() {
		...
		JLabel lblNewLabel = new JLabel("Update Vaccination Status");
		lblNewLabel.setHorizontalAlignment(SwingConstants.CENTER);
		lblNewLabel.setFont(new Font("Arial Narrow", Font.BOLD, 45));
		lblNewLabel.setBounds(197, 11, 510, 66);
		...
		updateTestResultUpdate_btn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
				    ...
				    PreparedStatement ps = conn.prepareStatement("update covid19_management_application.vaccination set result =?, labid =? where phno = ?;");
		...
		scrollPane.setViewportView(vaccinationtable);
		vaccinationtable.setModel(new DefaultTableModel(
			new Object[][] {
			},
			new String[] {
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					Class.forName("com.mysql.cj.jdbc.Driver");
				    Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/covid19_management_application","root","password@mysql");
				    String query = "SELECT phno,name,labarea,result FROM covid19_management_application.vaccination where result = 'tbd'";
				    PreparedStatement ps = conn.prepareStatement(query);
				    ResultSet rs = ps.executeQuery();
				    vaccinationtable.setModel(DbUtils.resultSetToTableModel(rs));
				}catch(Exception e4)
				{
					JOptionPane.showMessageDialog(null,"Duplicate entry or all the fields have not been entered.");
				}
